      ...
      <v-row>
        ...
        <v-col cols="12" md="9">
          ...
          >
            ...
            <template v-slot:item.max-oh="{ item }">
              <div align="right">{{ item.artnr == 0 ? "" : formatNumber(item['max-oh'].toFixed(2)) }}</div>
            </template>
            ...
            <template v-slot:item.datum="{ item }">
              <div align="left" v-if="item.datum != null">{{ item.artnr == 0 ? "" : formatDate(item.datum) }}</div>
            </template>
          </v-data-table>
        </v-col>
      </v-row>
...
import MaxOH from "@/api/inventory/report/max-stock-on-hand.api";
...
export default {
  ...
  data: () => {
    return {
      headers: [
        ...
        { text: "Maximum On-Hand", align: "center", value: "max-oh" },
  ...
  methods: {
    search() {
      requestBody.sorttype = this.sortBy;
      requestBody.mainGrp = this.fromMainGroup;
      requestBody.fromStore3 = this.fromStorage;
      requestBody.toStore3 = this.toStorage;
      (async () => {
        const respons = await MaxOH.loadData(htlUrl, requestBody);
        const ohList = respons.response.maxOnhandList["max-onhand-list"];
        this.datas = ohList;
      })();
    },
    loadStorage() {
      (async () => {
        const respons = await MaxOH.prepareData(htlUrl, requestBody);
        requestBody.showPrice = respons.response.showPrice;
    ...
    loadMainGroup() {
      (async () => {
        const respons = await MaxOH.prepareData(htlUrl, requestBody);
        const tempMainGroup = respons.response.tLHauptgrp["t-l-hauptgrp"];
