...
import MinOH from "@/api/inventory/report/min-stock-on-hand.api";
...
export default {
  ...
  methods: {
    search() {
      ...
      (async () => {
        const respons = await MinOH.loadData(htlUrl, requestBody);
        const ohList = respons.response.minOnhandList["min-onhand-list"];
        this.datas = ohList;
      })();
    },
    loadStorage() {
      (async () => {
        const respons = await MinOH.prepareData(htlUrl, requestBody);
