<template>
  <div>
    <q-drawer :value="true" side="left" bordered :width="220" persistent>
      <SearchChartIssuedwithoutPO @onSearch="onSearch" />
    </q-drawer>
    <div class="q-pa-lg">
      <div class="q-mb-md">
...
import { tableHeaders } from './tables/Issuedwithout.table';
...
export default defineComponent({
  setup(_, { root: { $api } }) {
    ...
    onMounted(async () => {
      const GET_DATA = await Promise.all([
        $api.inventory.apiIssuedwithoutPO('directIssueWithPOPrepare'),
      ]);
  ...
  components: {
    SearchChartIssuedwithoutPO: () =>
      import('./components/SearchIssuedwithoutpo.vue'),
  },
});
</script>
