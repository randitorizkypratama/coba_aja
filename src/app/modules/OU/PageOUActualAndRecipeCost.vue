  ...
  setup(_, { root: { $api } }) {
    ...
    onMounted(async () => {
      const [data] = await Promise.all([
        $api.outlet.getOUPrepareActualAndRecipeCost('fbCostAnalPrepare', {}),
      ]);

      responsePrepare = data || [];
      state.dataPrepare = responsePrepare; 

      // fromdate todate not yet assign , on dataPrepare
    ...
    const onSearch = (state2) => {
        ...
        async function asyncCall() {
            const dataResponse = await Promise.all([
                $api.outlet.getOUActualAndRecipeCost('fbCostAnalList', {
