...
export default {
  ...
  methods: {
    onClickSearch() {
      if ( (!this.$refs.form.validate() && this.dataForm.showAllUser) || (this.$refs.form.validate()) ) {
        this.getDataTable();
      } 
    }, getDataPrepare() {
      (async () => {
        ...
        if (!response.ok) {
          ...
          this.dataUser = this.mapDataInArray(this.dataPrepare.userList['user-list'], "usrnr", "usrnr", "usrname");
